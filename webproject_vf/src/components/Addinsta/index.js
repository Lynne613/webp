...
class index extends Component {
  ...
  render() {
    return (
      <Modal size='small' dimmer={"blurring"} trigger={<Button inverted color='purple' onClick={this.handleOpen} >上傳貼文</Button>}
        open={this.state.modalOpen}
        onClose={this.handleClose}
      >
        <Modal.Header>
          <div className="upload-btn-wrapper">
            <button className="btn">上傳照片</button>
        ...
        <Modal.Content image>
          ...
          <Modal.Description  className="descriptionTxtArea">
           ...
           <TextArea onChange={event => this.handleOnchange(event)}
             className="" autoHeight placeholder='請上傳美美的照片'
        ...
        <Modal.Actions>
          {(this.state.uploading === false) ? (
            <Button content='PO文' disabled onClick={this.uploadHandler} danger />
          ) : (
              <Button content='PO文' onClick={this.uploadHandler} primary />
            )}
